...
function processValueChanged(variables, oldVariable, newVariable) {
    ...
    if (newVariable && oldVariable && oldVariable.value !== newVariable.value) {
        console.log('Variable changed from ' + oldVariable.value + ' to ' + newVariable.value);
    ...
    if (newVariable && newVariable.type == 'number') {
       ...
       getTrigger('if_one_of_variable_changed').trigger(null, { "variable": newVariable.name, "value": newVariable.value }, { oldVariable: oldVariable, newVariable: newVariable });
    }
