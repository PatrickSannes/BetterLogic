...
function processValueChanged(variables,oldvariable, newVariable) {
    ...
    if ((oldvariable && !newVariable) || (oldvariable && newVariable && oldvariable.hasInsights && !newVariable.hasInsights )) {
    ...
    if (newVariable && newVariable.hasInsights && !logExists(newVariable.name) ) {
        Homey.manager('insights').createLog(newVariable.name, { 'en': 'Variable' }, variable.type, { 'en': '' }, function (err, state) { });
...
function logExists(variableName) {
    insights.forEach(function(variable) {
