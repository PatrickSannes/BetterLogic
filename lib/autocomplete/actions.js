...
exports.createAutocompleteActions = function () {
    ...
    Homey.manager('flow').on('action.set_a_variable.variable.autocomplete', function (callback, value) {
        callback(null, variableManager.getVariables().filter(util.contains(value)));
    });
    Homey.manager('flow').on('action.execute_expression.variable.autocomplete', function (callback, value) {
        callback(null, variableManager.getVariables().filter(util.filterVariable(value, 'number')));
    });
    Homey.manager('flow').on('action.trigger_variable.variable.autocomplete', function (callback, value) {
        callback(null, variableManager.getVariables().filter(util.filterVariable(value, 'trigger')));
    });
}